 ...
 * instant.
 *
 * NB: expo-font is a NATIVE module. Adding/removing it requires a clean native
 * rebuild (expo prebuild --clean) with the Expo SDK patch set aligned, or the
 * app crashes at launch with a DYLD "symbol not found" between ExpoFont and
 * ExpoModulesCore. See docs / PR notes. */
...
export function applyHeebo(): void {
  ...
  for (const Comp of [Text, TextInput] as any[]) {
    try {
      ...
      // Leave this component on the system font rather than risk breaking screens.
    }
  }
}

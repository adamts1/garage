/* @garage/shared — everything the web board and the phone app both rely on.
   ...
   The last of those four is the subtle one. `intake.ts` and `waMessage.ts` hold
   Hebrew strings, which looks like copy and is not: the first is written into
   rows and read back by the other app, the second is sent to a customer. Neither
   may live in an app's translation file, where editing a label would change
   stored data or change what a garage sends out. Text the *operator* reads stays
   in each app's own locale file. */
